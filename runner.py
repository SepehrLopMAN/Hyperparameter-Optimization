...
────────────────────────────────────────────────────────────────
    ...
    POP_SIZE = 1024   Large population is justified by GPU parallelism:
                      evaluating 1024 chromosomes costs almost the same GPU time
                      as evaluating 32 (kernel launch dominates, not compute).
                      More chromosomes → better initial coverage → less re-runs.
...
CSV_OUT  = f"GA_{DIM}-{POP_SIZE}_{DEVICE_NAME}_benchmark_results.csv"
