"""
runner.py  —  GEO Benchmark Runner
=====================================
...
────────────────────────────────────────────────────────────────
    ...
    POP_SIZE = 1024   Large population is justified by GPU parallelism:
                      evaluating 1024 eagles costs almost the same GPU time
                      as evaluating 32 (kernel launch dominates, not compute).
                      More eagles → better initial coverage → less re-runs.
...
CSV_OUT  = f"GEO_{DIM}-{POP_SIZE}_{DEVICE_NAME}_benchmark_results.csv"
