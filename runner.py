"""
runner.py  —  L-SHADE Benchmark Runner
=====================================
...
────────────────────────────────────────────────────────────────
    ...
    POP_SIZE = 1024   Large population is justified by GPU parallelism:
                      evaluating 1024 individuals/agents costs almost the same GPU time
                      as evaluating 32 (kernel launch dominates, not compute).
                      More individuals/agents → better initial coverage → less re-runs.
...
CSV_OUT  = f"L-SHADE_{DIM}-{POP_SIZE}_{DEVICE_NAME}_benchmark_results.csv"
...
def main():
    ...
    t_total = time.time()
    print(SEP)
    print(f"  L-SHADE Benchmark Suite")
